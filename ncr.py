...
# Disable SSL warnings (Corrected line)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
...
def fetch_project_data(session_id, project_name, form_name, record_limit=1000):
    ...
    with st.spinner("Fetching data from Asite..."):
        while True:
            search_criteria = {
                "criteria": [{"field": "ProjectName", "operator": 1, "values": [project_name]}, {"field": "FormName", "operator": 1, "values": [form_name]}],
                "recordStart": start_record,
                "recordLimit": record_limit
            }
            search_criteria_str = json.dumps(search_criteria)
...
def generate_ncr_report(df, report_type, start_date=None, end_date=None):
    with st.spinner(f"Generating {report_type} NCR Report..."):
        ...
        for i in range(0, len(cleaned_data), chunk_size):
            ...
            payload = {
                "input": prompt,
                "parameters": {"decoding_method": "greedy", "max_new_tokens": 8100, "min_new_tokens": 0, "temperature": 0.01},
                "model_id": MODEL_ID,
                "project_id": PROJECT_ID
            }
                    ...
                    if generated_text:
                        ...
                        if json_match:
                            ...
                            try:
                                ...
                                for site, counts in chunk_sites.items():
                                    if site not in all_results[report_type]["Sites"]:
                                        all_results[report_type]["Sites"][site] = {"SW": 0, "FW": 0, "MEP": 0, "Total": 0}
                                    all_results[report_type]["Sites"][site]["SW"] += counts.get("SW", 0)
                                    all_results[report_type]["Sites"][site]["FW"] += counts.get("FW", 0)
                                    all_results[report_type]["Sites"][site]["MEP"] += counts.get("MEP", 0)
                                    all_results[report_type]["Sites"][site]["Total"] += counts.get("Total", 0)
...
def generate_consolidated_ncr_excel(combined_result, report_title="NCR"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        site_format = workbook.add_format({'align': 'left', 'valign': 'vcenter', 'border': 1})
        worksheet = workbook.add_worksheet('NCR Report')
        ...
        for site in all_sites:
            ...
            if match:
                num = match.group(1)
                normalized_sites[site] = f"Veridia- Tower {num.zfill(2)}"
            else:
                normalized_sites[site] = site
        standard_sites = ["Veridia-Club", "Veridia- Tower 01", "Veridia- Tower 02", "Veridia- Tower 03", "Veridia- Towers 04 (A)", "Veridia- Tower 04 (B)", "Veridia- Tower 05", "Veridia- Tower 06", "Veridia- Tower 07", "Veridia-Commercial", "External Development"]
        for site in standard_sites:
            if site not in normalized_sites.values():
                normalized_sites[site] = site
        sorted_sites = sorted(list(set(normalized_sites.values())))
        ...
        for site in sorted_sites:
            worksheet.write(row, 0, site, site_format)
            original_resolved_key = next((orig_site for orig_site, norm_site in normalized_sites.items() if norm_site == site and orig_site in resolved_sites), None)
            original_open_key = next((orig_site for orig_site, norm_site in normalized_sites.items() if norm_site == site and orig_site in open_sites), None)
            site_total = 0
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                value = resolved_sites.get(original_resolved_key, {}).get(json_cat, 0) if original_resolved_key else 0
                worksheet.write(row, i+1, value, cell_format)
                site_total += value
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                value = open_sites.get(original_open_key, {}).get(json_cat, 0) if original_open_key else 0
                worksheet.write(row, i+4, value, cell_format)
                site_total += value
            worksheet.write(row, 7, site_total, cell_format)
            site_totals[site] = site_total
